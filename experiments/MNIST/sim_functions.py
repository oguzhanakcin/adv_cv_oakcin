...
def ood_performance(data,device,hyp):
    sim = Scodselect(data,hyp,device)

    sim.prepro()

    test_loader = torch.utils.data.DataLoader(sim.test_dataset, batch_size=1)
    data = torch.zeros((1, 2), dtype=torch.float).to(sim.device)
    label = torch.zeros((1), dtype=int).to(sim.device)

    sim.model1.eval()
    sim.model2.eval()
    sim.model3.eval()

    uncs1 = [[] for i in range(7)]
    uncs2 = [[] for i in range(7)]
    uncs3 = [[] for i in range(7)]
    ...
    for test_data in test_loader:
        ...
        emb1, out1, unc1 = sim.scod1(data)
        emb2, out2, unc2 = sim.scod2(data)
        emb3, out3, unc3 = sim.scod3(data)

        uncs1[int(label[0])].append(unc1.tolist()[0])
        uncs2[int(label[0])].append(unc2.tolist()[0])
        uncs3[int(label[0])].append(unc3.tolist()[0])

    k1 = plt.figure(figsize=(10, 10))

    plt.hist(uncs1,label=[i for i in range(7)])
    plt.legend()
    plt.title("Unc Scores for 0,1 Training data")
    k2 = plt.figure(figsize=(10, 10))

    plt.hist(uncs2, label=[i for i in range(7)])
    plt.legend()
    plt.title("Unc Scores for 0,2 Training data")
    k3 = plt.figure(figsize=(10, 10))

    plt.hist(uncs3, label=[i for i in range(7)])
    plt.legend()
    plt.title("Unc Scores for 0,3 Training data")
    plt.show()
...
def train_base(data,device,hyp,hypgen):
    ...
    for i in range(n_device):
        ...
        confusion_matrix = test_MNIST_function(base_model,test_dataset,device)
...
def simulate(data,device,hyp,hypgen,out_loc,sim_type):
    ...
    if sim_type == "random":
        sim = MNISTRandom(data,hyp, hypgen,device)
    elif sim_type == "oracle":
        sim = MNISTOracle(data,hyp, hypgen,device)
    ...
    for i in bar:
        ...
        if (sim_type == "scod_gu" or sim_type == "gu"):
            sim.gu_train(i)
