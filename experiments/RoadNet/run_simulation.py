...
def run_sim(hyp,opt,device):
    ...
    for i in range(4):
        ind = [j for j in range(len(train_labels)) if train_labels[j]==i]
        f_ind = ind[:opt.num_samples]
        train_locs += [train_imgs_locs[j] for j in f_ind]
        train_ls += [train_labels[j] for j in f_ind]
    ...
    for i in range(4):
        ind = [j for j in range(len(test_labels)) if test_labels[j]==i]
        f_ind = ind[:int(opt.num_samples/10)]
        test_locs += [test_imgs_locs[j] for j in f_ind]
        test_ls += [test_labels[j] for j in f_ind]
    

    data = [train_locs,train_ls,test_locs,test_ls]
