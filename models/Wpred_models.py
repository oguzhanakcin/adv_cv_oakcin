...
class Wpred:
    def __init__(self, data, hyp,hypgen, device,dataset_loc):
        ...
        self.train_dataset = create_Weather_dataset(self.X_train, self.y_train,dataset_loc)
        ...
        for i in range(self.n_device):
            ...
            if hypgen["model_size"] == "small":
                model_i = vsmodels.resnet18().to(device)
                set_parameter_requires_grad(model_i,True)
            ...
            elif hypgen["model_size"] == "medium" :
                model_i = vsmodels.resnet50().to(device)
                set_parameter_requires_grad(model_i, True)
                model_i.fc = nn.Linear(in_features=2048, out_features=4).to(device)
                model_i.load_state_dict(torch.load(hyp["weight_loc"] + "mediumbasemodel" + str(i) + ".pt"))
            else:
                model_i = vsmodels.resnet101().to(device)
                set_parameter_requires_grad(model_i, True)
                model_i.fc = nn.Linear(in_features=2048, out_features=4).to(device)
                model_i.load_state_dict(torch.load(hyp["weight_loc"] + "largebasemodel" + str(i) + ".pt"))
